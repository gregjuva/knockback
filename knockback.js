...
Knockback.CollectionObservable = (function() {
  ...
  function CollectionObservable(collection, vm_observable_array, options) {
    var defer, event, _i, _j, _len, _len1, _ref, _ref1;
  ...
  CollectionObservable.prototype.sortedIndex = function(sorted_index, sort_attribute, options) {
    ...
    _resync = function() {
      if ((_this._kb_collection.models.length === 0) && (_this._kb_value_observable().length === 0)) {
        return;
      }
      _this._collectionResync(true);
      if (!options.silent) {
        return _this.trigger('resort', _this.vm_observable_array());
      }
    };
    if (options.defer) {
      _.defer(_resync);
  ...
  CollectionObservable.prototype._collectionResync = function(silent) {
    var add_index, model, models, view_models, _i, _j, _len, _len1, _ref;
  ...
  function DefaultWrapper(observable, default_value) {
    ...
    this._kb_observable = ko.dependentObservable({
      read: function() {
        ...
        if (!value) {
          return ko.utils.unwrapObservable(_this.default_value);
        } else {
          return value;
        }
      },
...
Knockback.Observables = (function() {
  ...
  function Observables(model, mappings_info, view_model, options_or_writeable) {
    var is_string, mapping_info, view_model_property_name, write, _ref, _ref1;
    this.model = model;
...
AttributeConnector = (function() {
  ...
  function AttributeConnector(model, key, read_only) {
    ...
    this._kb_observable.subscription = this._kb_observable.subscribe(function(value) {
      var set_info;
      if (_this.read_only) {
        if (_this.model) {
          value = _this.model.get(_this.key);
          if (_this._kb_observable() === value) {
            return;
          }
          _this._kb_observable(value);
        }
        throw "Cannot write a value to a dependentObservable unless you specify a 'write' option. If you wish to read the current value, don't pass any parameters.";
      } else if (_this.model) {
        set_info = {};
        set_info[_this.key] = value;
        return _this.model.set(set_info);
      }
    });
    this._kb_observable.destroy = this.destroy;
  ...
  ViewModel.prototype._kb_vm_onModelChange = function() {
    ...
    if (this._kb_vm.model._changed) {
      ...
      for (key in this._kb_vm.model.changed) {
        _results1.push(this._updateAttributeObservor(this._kb_vm.model, key));
      }
      return _results1;
    }
  };
